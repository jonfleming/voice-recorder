...
#if defined(CONFIG_FATFS_LONG_FILENAMES) && !CONFIG_FATFS_LONG_FILENAMES
    ESP_LOGW(TAG, "Warning: Long filenames on SD card are disabled in menuconfig!");
#endif
#if !defined(CONFIG_FATFS_LFN_HEAP) || !CONFIG_FATFS_LFN_HEAP
    // no warning - HEAP case handled via LONG_FILENAMES above; IDF 5.5 uses LFN_HEAP
#endif
...
{
    // Touch controller POR needs ~100ms after display power-on; serial monitor during plug-in adds USB enumeration delay
    vTaskDelay(pdMS_TO_TICKS(150));
    esp_err_t touch_ret = ESP_OK;
    for (int attempt = 0; attempt < 5; attempt++) {
        if (attempt > 0) {
            ESP_LOGW(TAG, "Touch init failed (0x%x), retry %d/5 with bus recovery...", touch_ret, attempt);
            // I2C bus recovery: deinit/reinit with pull-ups, then delay for touch controller POR
            bsp_i2c_deinit();
            vTaskDelay(pdMS_TO_TICKS(80));
            bsp_i2c_init();
            vTaskDelay(pdMS_TO_TICKS(150 + attempt * 50));
        }
        tp = NULL;
        touch_ret = bsp_touch_new(NULL, &tp);
        if (touch_ret == ESP_OK && tp != NULL) {
            if (attempt > 0) ESP_LOGI(TAG, "Touch recovered on attempt %d", attempt + 1);
            break;
        }
        vTaskDelay(pdMS_TO_TICKS(60));
    }
    if (touch_ret != ESP_OK || tp == NULL) {
        ESP_LOGE(TAG, "Touch not found after 5 retries - continuing without touch (use buttons/serial)");
        tp = NULL;
        return NULL;
    }
    assert(tp);
    ...
    if (!indev) {
        ESP_LOGE(TAG, "lvgl_port_add_touch failed - continuing without touch");
    }
    return indev;
}
...
lv_display_t *bsp_display_start_with_config(const bsp_display_cfg_t *cfg)
{
    ...
    if (!disp_indev) {
        ESP_LOGW(TAG, "Display started without touch input - UI will be button/serial only");
    }
